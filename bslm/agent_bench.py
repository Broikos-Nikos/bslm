...
def main():
    ...
    labels = {}
    try:
        for l in (ROOT / "corpus" / "agent" / "facts.jsonl").open(encoding="utf-8"):
            x = json.loads(l)
            labels[(x["rel"], norm(x["subject"]))] = x.get("labels", 0)
    except OSError:
        pass
    known = [0, 0]                             # facts about subjects with an article in 40 or more languages
    per = defaultdict(lambda: [0, 0])          # correct, n  (with the runtime check)
    per_raw = defaultdict(int)                 # correct, the model alone
    quote_ok = [0, 0]                          # facts where the quote the model read carried the answer
        ...
        for i, task in enumerate(chosen):
            ...
            if task["family"] == "fact":
                ...
                if task.get("answer"):
                    quote_ok[1] += 1
                    q = r.get("quote", "")
                    quote_ok[0] += bool(q) and (found_in(task["answer"], q) or any(found_in(x, q) for x in task.get("aliases", [])))
                if labels.get((task["rel"], norm(task["subject"])), 0) >= 40:
                    known[1] += 1
                    known[0] += ok
...
Facts about well known subjects (an article in 40 or more languages): {known[0]}/{known[1]}{f" = {100 * known[0] / known[1]:.0f}%" if known[1] else ""}.
"Wikipedia said this": the quote the model read carried the answer in {quote_ok[0]}/{quote_ok[1]} findable facts{f" = {100 * quote_ok[0] / quote_ok[1]:.0f}%" if quote_ok[1] else ""}, the score a plain quoted answer would get.
